...
import { getProjects, getProjectMembers } from "../api/projectsApi.js";
import { getTasksByProject } from "../api/tasksApi.js";
export default function Scores() {
    ...
    useEffect(() => {
        ...
        getProjects(token).then(async (projects) => {
            const scores = await Promise.all(projects.map(async (p) => {
                const [tasks, members] = await Promise.all([
                    getTasksByProject(p.projectid, token),
                    getProjectMembers(p.projectid, token)
                ]);
                const projectMembersScores = members.map((member) => {
                    const memberId = member.userid || member.userId;
                    const memberTasks = tasks.filter((t) => (t.ownerid || t.ownerId) === memberId);
                    const approved = memberTasks.filter((t) => t.status === 'APPROVED');
    ...
    if (loading)
        return <div className="page-wrapper"><div className="text-center">Calculating scores...</div></div>;
    return (<div className="page-wrapper fade-in">
      ...
      {projectScores.length === 0 ? (<div className="card text-center" style={{ borderStyle: "dashed", padding: "64px" }}>
          <p style={{ color: "var(--color-text-tertiary)" }}>You are not in any projects yet.</p>
        </div>) : (<div style={{ display: "flex", flexDirection: "column", gap: "40px" }}>
          {projectScores.map(project => (<div key={project.id}>
              <h2 style={{ fontSize: "1.5rem", fontWeight: 700, marginBottom: "20px", color: "var(--color-primary)" }}>
                {project.name}
              </h2>
              <div className="grid grid-cols-2">
                {project.members.map((m) => (<div key={m.memberId} className="card accent-border">
