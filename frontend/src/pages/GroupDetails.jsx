...
import { getTasksByProject, updateTaskStatus, approveTask, createTask, getProjectActivity, } from "../api/tasksApi.js";
import { searchUsers } from "../api/usersApi.js";
import { addProjectMember, getProjectMembers, getProjectById, } from "../api/projectsApi.js";
import ConfirmModal from "../components/ConfirmModal.jsx";
import { useConfirmModal } from "../hooks/useConfirmModal.js";
import { useToast } from "../context/ToastContext.jsx";
import { Button } from "../components/Button.jsx";
import { FormInput } from "../components/FormInput.jsx";
const TABS = ["TASKS", "MEMBERS", "ACTIVITY", "SCORES"];
export default function GroupDetail() {
    ...
    const handleCreateTask = async () => {
        if (!token || !groupId || !newTaskTitle.trim() || creatingTask)
            return;
        if (!newTaskAssignee) {
        ...
        try {
            await createTask(groupId, newTaskTitle, newTaskDeadline, token, newTaskAssignee);
            setNewTaskTitle("");
            setNewTaskDeadline("");
            setNewTaskAssignee("");
    ...
    if (loading)
        return <div className="page-wrapper"><div className="text-center">Loading Group Data...</div></div>;
    ...
    return (<div className="page-wrapper fade-in">
      <div style={{ marginBottom: "2rem" }}>
        ...
        <div className="glass" style={{
          ...
          {TABS.map((tab) => (<button key={tab} onClick={() => setActiveTab(tab)} className="btn" style={{
      ...
      {/* TASKS TAB */}
      {activeTab === "TASKS" && (<div className="animate-fade">
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "24px" }}>
            <h2 style={{ fontSize: "1.5rem", fontWeight: 700 }}>Tasks</h2>
            {isProjectOwner && (<Button variant={showTaskForm ? "secondary" : "primary"} onClick={() => setShowTaskForm(!showTaskForm)}>
                {showTaskForm ? "Cancel" : "+ Create Task"}
              </Button>)}
          </div>

          {showTaskForm && (<div className="card glass-light" style={{ marginBottom: "32px", border: "1px solid var(--color-primary-light)" }}>
              <h3 style={{ marginBottom: "20px" }}>Create New Task</h3>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px", marginBottom: "20px" }}>
                <FormInput label="Task Title" placeholder="What needs to be done?" value={newTaskTitle} onChange={(e) => setNewTaskTitle(e.target.value)}/>
                <FormInput label="Deadline" type="datetime-local" value={newTaskDeadline} onChange={(e) => setNewTaskDeadline(e.target.value)}/>
              </div>
              <div style={{ marginBottom: "24px" }}>
                <label className="form-label">Assign to:</label>
                <select value={newTaskAssignee} onChange={(e) => setNewTaskAssignee(e.target.value)} className="form-input" style={{ borderColor: newTaskAssignee ? "var(--color-border)" : "var(--color-danger)" }}>
                  <option value="">-- Select a member --</option>
                  {members.map((member) => (<option key={member.userid || member.userId} value={member.userid || member.userId}>
                      {member.name} ({member.email})
                    </option>))}
                </select>
              </div>
              <Button onClick={handleCreateTask} disabled={creatingTask || !newTaskTitle.trim()} loading={creatingTask} variant="success">
                Create Task
              </Button>
            </div>)}

          {tasks.length === 0 && (<p style={{ color: "#666", fontStyle: "italic" }}>
          ...
          {tasks.map((task) => {
                ...
                return (<div key={task.taskid || task.taskId} className="card animate-fade" style={{ marginBottom: "16px" }}>
                ...
                <div style={{ display: "flex", gap: "24px", color: "var(--color-text-secondary)", fontSize: "0.9rem" }}>
                  ...
                  {task.deadline && (<div style={{ color: isOverdue ? "var(--color-danger)" : "inherit", fontWeight: isOverdue ? 700 : 400 }}>
                ...
                <div style={{ marginTop: "20px", display: "flex", gap: "10px" }}>
                  {taskOwnerId === user?.id && task.status === "CREATED" && (<Button variant="primary" size="sm" onClick={() => askConfirm("Start Task", "Are you sure you want to start this task?", () => handleStatusChange(task.taskid || task.taskId, "IN_PROGRESS"), "primary")}>
                      Start Task
                    </Button>)}

                  {taskOwnerId === user?.id && task.status === "IN_PROGRESS" && (<Button variant="success" size="sm" onClick={() => askConfirm("Mark Task Done", "Are you sure you have completed this task?", () => handleStatusChange(task.taskid || task.taskId, "DONE"), "success")}>
                      Mark as Done
                    </Button>)}

                  {isProjectOwner && task.status === "DONE" && (<Button variant="success" size="sm" onClick={() => askConfirm("Approve Task", "Are you sure you want to approve this task completion?", () => handleApprove(task.taskid || task.taskId), "success")}>
                      Approve Task
                    </Button>)}
                </div>
              </div>);
      ...
      {/* MEMBERS TAB */}
      {activeTab === "MEMBERS" && (<div className="animate-fade">
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "24px" }}>
            <h2 style={{ fontSize: "1.5rem", fontWeight: 700 }}>Members</h2>
            {isProjectOwner && (<Button variant={showMemberForm ? "secondary" : "primary"} onClick={() => setShowMemberForm(!showMemberForm)}>
                {showMemberForm ? "Cancel" : "+ Add Member"}
              </Button>)}
          </div>

          {showMemberForm && (<div className="card glass" style={{ marginBottom: "32px", border: "1px solid var(--color-primary-light)" }}>
              <h3 style={{ marginBottom: "20px" }}>Add Member to Group</h3>
              <div style={{ display: "flex", gap: "12px", alignItems: "flex-end" }}>
                <FormInput label="Search Users" placeholder="name or email" value={userSearch} onChange={(e) => setUserSearch(e.target.value)} style={{ flex: 1, marginBottom: 0 }}/>
                <Button onClick={handleSearchUsers}>Search</Button>
              </div>

              {searchResults.length > 0 && (<div style={{ marginTop: "24px" }}>
                  <h4 style={{ fontSize: "1rem", color: "var(--color-text-secondary)", marginBottom: "12px" }}>Search Results:</h4>
                  {searchResults.map((result) => (<div key={result.id} className="card" style={{
                            padding: "12px 20px",
                            marginBottom: "10px",
                            display: "flex",
          ...
          <div style={{ marginTop: "20px" }}>
            ...
            <div className="grid grid-cols-2">
              {members.map((member) => (<div key={member.userid || member.userId} className="card">
      ...
      {/* ACTIVITY TAB */}
      {activeTab === "ACTIVITY" && (<div className="animate-fade">
          <h2 style={{ fontSize: "1.5rem", fontWeight: 700, marginBottom: "24px" }}>Group Activity</h2>
          {activity.length === 0 ? (<div className="card text-center" style={{ borderStyle: "dashed", padding: "48px" }}>
              ...
              {activity.map((event) => {
                    ...
                    return (<div key={event.event_id} style={{ marginBottom: "32px", position: "relative" }}>
                    <div style={{
                            position: "absolute",
                            left: "-39px",
                            top: "4px",
                    ...
                    <div style={{
                      ...
                      {event.type === "TASK_STATUS_CHANGED" && (<span>
                          {" ✏️ Updated "}
                          <strong style={{ color: "#555" }}>
                      ...
                      {event.type === "TASK_APPROVED" && (<span>
                          {" ✅ Approved: "}
                          <strong style={{ color: "#555" }}>
                            {event.metadata?.taskTitle || "a task"}
                          </strong>
                        </span>)}
                      {event.type === "MEMBER_ADDED" && (<span>
                          {" 👥 Added Member: "}
                          <strong style={{ color: "#555" }}>
      ...
      {/* SCORES TAB */}
      {activeTab === "SCORES" && (<div className="animate-fade">
          ...
          <div className="grid grid-cols-2">
            {members.map((member) => {
                ...
                const score = memberTasks.length > 0
                    ...
                    : 0;
                return (<div key={member.userid} className="card">
