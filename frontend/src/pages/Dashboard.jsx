...
import { getMyTasks, updateTaskStatus, approveTask } from "../api/tasksApi.js";
...
export default function Dashboard() {
    ...
    const activeTasks = tasks.filter((t) => t.status !== "APPROVED" && t.status !== "CANCELLED");
    return (<div className="page-wrapper fade-in">
      ...
      <div className="grid grid-cols-2">
        <section className="animate-fade">
          ...
          {tasks.length === 0 ? (<div className="card text-center" style={{ borderStyle: "dashed", padding: "48px" }}>
              ...
              {tasks.slice(0, 5).map((task) => {
                ...
                return (<div key={task.taskid} className="card" style={{ transition: "transform 0.2s ease" }}>
                    ...
                    <div style={{ display: "flex", gap: "16px", marginBottom: "16px", fontSize: "0.9rem" }}>
                      ...
                      {task.deadline && (<span style={{ color: isOverdue ? "var(--color-danger)" : "var(--color-text-tertiary)" }}>
                    ...
                    <div style={{ display: "flex", gap: "12px" }}>
                      {task.status === "CREATED" && (<Button size="sm" variant="primary" onClick={() => askConfirm("Start Task", "Are you sure?", () => handleStatusChange(task.taskid, "IN_PROGRESS"))}>
                          Start
                        </Button>)}
                      {task.status === "IN_PROGRESS" && (<Button size="sm" variant="success" onClick={() => askConfirm("Mark Done", "Completed this task?", () => handleStatusChange(task.taskid, "DONE"))}>
                          Mark Done
                        </Button>)}
                    </div>
                  </div>);
        ...
        <section className="animate-fade" style={{ animationDelay: "0.1s" }}>
          ...
          {projects.length === 0 ? (<div className="card text-center" style={{ borderStyle: "dashed", padding: "48px" }}>
              <p style={{ color: "var(--color-text-tertiary)" }}>👥 You are not in any groups yet.</p>
            </div>) : (<div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
              {projects.map((project) => (<Link key={project.projectid} to={`/groups/${project.projectid}`} style={{ textDecoration: "none" }}>
